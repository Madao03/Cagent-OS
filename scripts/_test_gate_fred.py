"""Test: Interest rate question — FRED data IS available, does agent fetch it?

This is the critical test of the gate: the data exists in the system (FRED),
the agent has the tools, but previously it answered from memory (5 hallucination).
The gate should force the agent to call FRED instead of answering from parametric knowledge.
"""
import sys, time
...
QUESTION = "现在美国的联邦基金利率和 10 年期国债收益率分别是多少？"
...
# Show registry facts count by kind
data_facts = [f for f in executor.fact_registry.facts if f.kind == "data"]
vc_facts = [f for f in executor.fact_registry.facts if f.kind == "verified_citation"]
print(f"\nRegistry: {len(executor.fact_registry.facts)} total ({len(data_facts)} data, {len(vc_facts)} text)")
