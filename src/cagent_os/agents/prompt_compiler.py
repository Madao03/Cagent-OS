...
规则：
- ★以下情形必须写入 [derivations] 块，不得直接输出结果：
  - 同比 / 环比 / 增速（如「营收增长 23.2%」）
  - 比率（毛利率、净利率、PE、PS、CapEx/收入）
  - ★多项加总（如「三家合计 $110.8B」= a + b + c）
  - ★占比推算（如「其中 75% ≈ $450B」= total * 0.75）
  - ★涨跌幅（如「距52周高点 -10.3%」= (price - high) / high）
- 引用方式：`caliber@period`（如 `revenue@2025Q4`）或 fact_id（如 `f:0:3`），两种均可
- ★也可引用 quote.query 的字段：`price@MSFT`、`fifty_two_week_high@MSFT`、`two_hundred_day_avg@MSFT`
- 公式只允许：+, -, *, /, abs(), 括号 — 不要在公式里写数值计算（如 `(222.54亿 - 161.05亿) / 161.05亿`），只写引用
- 每条公式必须声明计算结果值
- 派生值不需要额外查工具——它们是对已有数据的数学运算
- 派生块放在输出最末尾，与正文空一行隔开
- ★即使输出很长、表格很宽，也必须声明所有派生值；不得因输出复杂而省略
